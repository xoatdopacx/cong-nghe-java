 ...
 * URL: /staff/diem (ADMIN hoặc STAFF truy cập)
 */
...
public class DiemController extends HttpServlet {
        ...
        switch (action) {
            ...
            case "edit":
                ...
                diemRepo.findById(id).ifPresentOrElse(
                    ...
                    () -> {
                        try { resp.sendRedirect(req.getContextPath() + "/staff/diem"); }
                        catch (Exception e) { throw new RuntimeException(e); }
                    }
                );
                break;
    ...
    private void listDiem(HttpServletRequest req, HttpServletResponse resp)
        ...
        req.getRequestDispatcher("/WEB-INF/views/staff/diem/list.jsp").forward(req, resp);
    }
    ...
    private void showForm(HttpServletRequest req, HttpServletResponse resp, Diem diem)
        ...
        req.getRequestDispatcher("/WEB-INF/views/staff/diem/form.jsp").forward(req, resp);
    }
}
