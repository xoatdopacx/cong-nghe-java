...
/**
 * Controller cho module Sản Phẩm.
 * URL: /staff/san-pham (ADMIN hoặc STAFF truy cập)
 */
@WebServlet(name = "SanPhamController", urlPatterns = "/staff/san-pham")
public class SanPhamController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        switch (action) {
            case "create":
                ...
                req.getRequestDispatcher("/WEB-INF/views/staff/sanpham/form.jsp").forward(req, resp);
                break;
            case "edit":
                ...
                spRepo.findById(id).ifPresentOrElse(
                    sp -> {
                        ...
                        try { req.getRequestDispatcher("/WEB-INF/views/staff/sanpham/form.jsp").forward(req, resp); }
                        catch (Exception e) { throw new RuntimeException(e); }
                    },
                    () -> {
                        try { resp.sendRedirect(req.getContextPath() + "/staff/san-pham"); }
                        catch (Exception e) { throw new RuntimeException(e); }
                    }
                );
                break;
    ...
    private void listSanPham(HttpServletRequest req, HttpServletResponse resp)
        ...
        req.getRequestDispatcher("/WEB-INF/views/staff/sanpham/list.jsp").forward(req, resp);
    }
}
