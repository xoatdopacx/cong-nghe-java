...
<div class="page-header">
    <h1>${isEdit ? '✏️ Sửa Điểm' : '➕ Nhập Điểm Mới'}</h1>
    <a href="${pageContext.request.contextPath}/diem" class="btn btn-secondary">← Quay lại</a>
</div>
    ...
    <form method="post" action="${pageContext.request.contextPath}/diem">
            ...
            <div class="form-group">
                ...
                <select id="sinhVienId" name="sinhVienId" class="form-control" required>
                    ...
                    <c:forEach var="sv" items="${sinhViens}">
                        <option value="${sv.id}"
                            ${diem.sinhVien != null && diem.sinhVien.id == sv.id ? 'selected' : ''}>
                            ${sv.maSV} - ${sv.hoTen}
                        </option>
                    </c:forEach>
                </select>
            ...
            <div class="form-group">
                ...
                <select id="monHocId" name="monHocId" class="form-control" required>
                    ...
                    <c:forEach var="mh" items="${monHocs}">
                        <option value="${mh.id}"
                            ${diem.monHoc != null && diem.monHoc.id == mh.id ? 'selected' : ''}>
                            ${mh.maMon} - ${mh.tenMon}
                        </option>
                    </c:forEach>
                </select>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" id="diemSo" name="diemSo" class="form-control"
                       value="${diem.diemSo}" min="0" max="10" step="0.1" required placeholder="VD: 8.5">
            </div>
            <div class="form-group">
                <label for="ghiChu">Ghi Chú</label>
                <input type="text" id="ghiChu" name="ghiChu" class="form-control"
                       value="${diem.ghiChu}" placeholder="VD: Thi lần 1">
            </div>
        </div>

        <div style="margin-top: 1rem;">
            <button type="submit" class="btn btn-success">${isEdit ? '💾 Cập Nhật' : '➕ Thêm Mới'}</button>
            <a href="${pageContext.request.contextPath}/diem" class="btn btn-secondary">Hủy</a>
        </div>
    </form>
</div>
