...
<div class="page-header">
    ...
    <a href="${pageContext.request.contextPath}/sinh-vien?action=create" class="btn btn-primary">➕ Thêm Sinh Viên</a>
</div>
...
<!-- Search -->
<form class="search-bar" method="get" action="${pageContext.request.contextPath}/sinh-vien">
    <input type="text" name="keyword" class="form-control" placeholder="🔍 Tìm theo mã SV, họ tên..."
           value="${keyword}">
    <button type="submit" class="btn btn-primary">Tìm kiếm</button>
    <c:if test="${not empty keyword}">
        <a href="${pageContext.request.contextPath}/sinh-vien" class="btn btn-secondary">Xóa lọc</a>
    </c:if>
</form>
        ...
        <tbody>
            <c:forEach var="sv" items="${sinhViens}" varStatus="loop">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${sv.lopHoc != null}">
                                <span class="badge badge-info">${sv.lopHoc.maLop}</span>
                            </c:when>
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <a href="${pageContext.request.contextPath}/sinh-vien?action=delete&id=${sv.id}"
                               ...
                               onclick="return confirm('Xóa sinh viên ${sv.hoTen}?')">🗑️ Xóa</a>
                        </div>
                    </td>
                </tr>
            </c:forEach>
...
<c:if test="${totalPages > 1}">
    <div class="pagination">
        <c:if test="${currentPage > 1}">
            <a href="${pageContext.request.contextPath}/sinh-vien?page=${currentPage-1}&keyword=${keyword}">« Trước</a>
        </c:if>
        <c:forEach begin="1" end="${totalPages}" var="i">
            <c:choose>
                <c:when test="${i == currentPage}"><span class="active">${i}</span></c:when>
                <c:otherwise>
                    <a href="${pageContext.request.contextPath}/sinh-vien?page=${i}&keyword=${keyword}">${i}</a>
                </c:otherwise>
            </c:choose>
        </c:forEach>
        <c:if test="${currentPage < totalPages}">
            <a href="${pageContext.request.contextPath}/sinh-vien?page=${currentPage+1}&keyword=${keyword}">Sau »</a>
        </c:if>
    </div>
</c:if>
